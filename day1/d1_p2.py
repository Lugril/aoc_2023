        ...
        if not digits:
            ...
            for num in numdic:
                ...
                if line[i:].startswith(num):
                    digits.append(numdic.get(num))
                    print ("detected", numdic.get(num))
                    break
        if sign.isdigit():
            digits.append(sign)
            last_dig_found = i
            print("detected", sign)
    end_cnt = 3
    print ("last_dig_found: ", last_dig_found)
    while (len(line)-end_cnt) > last_dig_found:
        for num in numdic:
            print ("line -end_cnt: ", line[-end_cnt:])
            if line[-end_cnt:].startswith(num):
                digits.append(numdic.get(num))
                print ("end detected", numdic.get(num))
                last_dig_found=len(line)
                break
        end_cnt += 1
    print("digits found in line: ", digits)
